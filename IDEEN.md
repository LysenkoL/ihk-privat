...
- **Azubi-Navigator и Prognose: печать, Markdown для Claude, баллы обратно, честная автопроверка** (`gen/azubi.js`, `gen/azubi.css`, `gen/pruefen.js`, `gen/wiederholen.js`, `tests/prognose.test.js`, 25.09):
  - под бланком и в Auswertung блок «Claude & Papier»: **Markdown für Claude** (весь бланк: ситуация, задания с таблицами, мои ответы с подписями полей, Musterlösung, Bewertungshinweis, id каждой подзадачи и просьба вернуть блок `PUNKTE`), **Nur Textantworten für Claude** (без того, что приложение уже проверило), **Als .md-Datei**, **Punkte einfügen** (ответ Claude целиком или только `PUNKTE`: строки `p1-1a: 5,5` или `1 a): 3`, полбалла, не больше максимума), **Drucken** — пусто с линиями для письма / с моими ответами / с ответами, решениями и баллами (A4, свой макет печати);
  - Markdown уходит тем же способом, что «Mit Claude prüfen» (чат этого экзамена / новый чат / только копировать);
  - **автопроверка полей стала честнее**: IP с префиксом (`10.40.7.128/26`), с подписью (`Netzadresse: …`), с пояснением в скобках, строка кода без пробелов/скобок; в числовых полях — результат после последнего «=» (`330/0,88 = 375 W`), часть до скобки, после «≈»; неверные ответы по-прежнему неверные (проверено на всех 46 автопроверяемых задачах пакета Azubi и всех задачах Prognose);
  - **«Mit Claude prüfen» в бланке Azubi/Prognose получал неполный ответ**: только текстовые поля без подписей, числовые поля терялись (Claude ставил «fehlt», хотя ответ был) и таблицы слипались — теперь Claude получает задание в Markdown с таблицами и весь ответ с подписями («Fehlerhafte Zeile: 4 / Erläuterung: …»), так же в «Fehler wiederholen»;
  - обновление кэша Service Worker до `ihk-ap1-v38-app`.
