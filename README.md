...
* **Офлайн PWA:** оболочка, генераторы и уже загруженные материалы кэшируются
  Service Worker v37; изображения целого экзамена надо заранее загрузить
  кнопкой «offline laden» на его карточке.

---

## Azubi-Navigator (u-form)

Раздел **«Azubi-Navigator (u-form)»** в группе «Prüfen & simulieren»: 10 Prüfungssimulationen
и 3 Vertiefende Übungen из личного аккаунта ausbildung.io — с сохранением на каждом вводе,
режимами Übung/Prüfung, автопроверкой закрытых заданий, самооценкой открытых по Musterlösung,
Auswertung с IHK-Note и архивом попыток.

* **Данные лицензионные и не публикуются:** `privat/azubi-daten.js` (в `.gitignore`).
  Собрать заново из экспорта: `python tools/azubi_import.py privat/azubi-navigator-export.json`.
...
## Endspurt, Fehler wiederholen, Fachbegriffe

* **Endspurt (последняя неделя):** план на каждый день до экзамена с учётом Azubi-Navigator —
  сегодня Übung, дальше по очереди Azubi-Prüfung (90 мин + Auswertung) и слабое место,
  за 3 дня Generalprobe (новейший экзамен IHK на бумаге), за 2 дня — все ошибки недели.
  Дни Azubi закрываются сами, когда модуль полностью оценён.
* **Fehler wiederholen:** все потерянные баллы (экзамены IHK, Azubi, генератор, карточки
  «nicht gewusst», каталог «unsicher») в одной очереди; не знала → завтра, знала → через
  2 дня, два раза подряд знала → из очереди (`ihk2:wieder`).
* **Fachbegriffe DE → RU:** 406 терминов с артиклем, переводом, простым объяснением и пометкой
  AP1 / AP1+2 / AP2; в текстах заданий подчёркнуты и открываются по тапу; поиск по-немецки
  и по-русски, карточки для заучивания (`ihk2:glossar:kann`).
* **Kurzfragen:** 153 коротких экзаменационных вопроса с образцом ответа и объяснением по-русски;
  режимы «Schreiben» (с проверкой), «Auswählen» (какой ответ даёт полные баллы) и «Aufdecken»
  (самооценка), выбор тем и количества; шаткие вопросы — в «Fehler wiederholen» (`ihk2:gen:satz`).
* **Nachschlagen:** любое слово в задании выделить → «nachschlagen»: термин, Prüfungsdeutsch
  (483 общих слова экзаменов с переводом, `gen/wortschatz-daten.js`) или свой перевод;
  «Wo kommt das vor?» показывает все места в IHK и Azubi; «+ Mein Glossar» — свои слова
  (`ihk2:glossar:mein`) в глоссарии, подчёркнуты лиловым.

---

## Prüfen lassen, Abgleich, SQL-Trainer

* **Prüfen lassen:** под каждым текстовым ответом — быстрая проверка (оператор, число пунктов,
  обоснование, Rechenweg и единица, ключевые слова Musterlösung) и кнопка «Mit Claude prüfen»:
  готовый промпт копируется, открывается claude.ai.
* **Geräte abgleichen:** «Stand senden» на одном устройстве, «Stand holen» на другом — оба стенда
  объединяются (ничего не перезаписывается), после обмена отчёт и «Rückgängig».
  Пакет Azubi-Navigator в файл не входит — его грузят на каждом устройстве один раз.
* **SQL-Trainer (AP2):** 44 задания на базе интернет-магазина, проверка по результату,
  немецкие пояснения к ошибкам, «Freies SQL». Библиотека `vendor/sqljs-1.14.2/`
  (sql.js, MIT) грузится при первом открытии и дальше работает офлайн. Через http(s)
  (GitHub Pages) берётся `sql-wasm.wasm`, при открытии `index.html` как файла — `sql-asm.js`
  (чистый JavaScript, WebAssembly из `file://` браузер не грузит).
...
* **Офлайн PWA:** интерфейс и уже открытые темы кэшируются Service Worker v32;
...
│   ├── pruefen.js          «Prüfen lassen»: Kurzcheck и промпт для Claude
│   ├── nachschlagen.js     выделить слово → перевод, где встречается, «Meine Wörter»
│   ├── wortschatz-daten.js Prüfungsdeutsch DE → RU (общие слова экзаменов)
│   ├── sync.js             обмен Handy ↔ Computer с объединением
│   ├── sql.js, sql-daten.js  SQL-Trainer (AP2)
...
├── vendor/sqljs-1.14.2/    SQLite для браузера (sql.js, MIT) — для SQL-Trainer
├── raw/                    исходные PDF
